...
func (d *Dao) GetTableColumns(ctx context.Context, schema, table string) ([]database.ColumnInfo, error) {
	...
	for rows.Next() {
		...
		var isNullable, isPK, isIdentity bool
		var defaultVal sql.NullString
		if err := rows.Scan(&col.Name, &col.DataType, &isNullable,
			&defaultVal, &isPK, &col.Ordinal, &col.Comment, &isIdentity); err != nil {
			return nil, err
		}
		col.IsNullable = isNullable
		col.IsPK = isPK
		col.IsAutoGenerated = isIdentity
		if defaultVal.Valid {
...
func (d *Dao) DefaultPKType() string { return "INT IDENTITY(1,1)" }
