...
// Dao implements database.Driver for MariaDB.
type Dao struct {
	client *Client
}
...
func (d *Dao) RenameColumn(ctx context.Context, schema, table, old, newName string) error {
	// RENAME COLUMN is available in MariaDB 10.5.2+.
...
func (d *Dao) DropIndex(ctx context.Context, schema, indexName string) error {
	// MariaDB requires the table name in DROP INDEX; look it up from information_schema.
...
// ExplainPlan uses EXPLAIN FORMAT=JSON (supported since MariaDB 10.0.5).
func (d *Dao) ExplainPlan(ctx context.Context, sqlStr string) (string, error) {
	return d.explainText(ctx, "EXPLAIN FORMAT=JSON "+sqlStr)
}

// ExplainAnalyze uses ANALYZE FORMAT=JSON (supported since MariaDB 10.1).
func (d *Dao) ExplainAnalyze(ctx context.Context, sqlStr string) (string, error) {
	return d.explainText(ctx, "ANALYZE FORMAT=JSON "+sqlStr)
}
